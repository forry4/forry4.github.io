"""Sweep: L1-ONLY cost-spread discount (W_SPREAD).

card_value /= (1 + W_SPREAD * cost_concentration) for level-1 cards only, so a
spread cost (2g+1r, conc 1) outranks a single-color one (3g, conc 2) of equal
value. Tests whether preferring spread-cost L1 cards helps. Paired vs W_SPREAD=0
on the SAME fresh seeds vs the A/B/C/C2 mix. ASCII output only (cp1252).
"""
import os
...
OPP_NAMES = ["A", "B", "C", "C2"]
SEEDS = list(range(57500, 59000))   # 1500 fresh seeds (disjoint from prior runs)
N_SHARDS = 16

CONFIGS = {
    "W_SPREAD 0.0 (ref)": {"W_SPREAD": 0.0},
    "W_SPREAD 0.1":       {"W_SPREAD": 0.1},
    "W_SPREAD 0.2":       {"W_SPREAD": 0.2},
    "W_SPREAD 0.4":       {"W_SPREAD": 0.4},
    "W_SPREAD 0.8":       {"W_SPREAD": 0.8},
}
REF = "W_SPREAD 0.0 (ref)"
...
if __name__ == "__main__":
    ...
    print(f"sweep W_SPREAD (L1-only) {list(CONFIGS)} x {len(SEEDS)} fresh games vs mix "
