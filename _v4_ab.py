"""Paired A/B harness for heuristic toggles/configs (reusable).

Compares named configs (each a dict of heuristic.py attribute overrides) on the
SAME fresh seeds vs the A/B/C/C2 mix, deterministic per-game RNG. Reports each
config's overall + per-opponent winrate and the PAIRED difference vs the first
config (each seed is the identical game, so config_b - config_a per seed is pure
signal). Seeds are sharded across workers for speed. ASCII output only (cp1252).
"""
import os
...
OPP_NAMES = ["A", "B", "C", "C2"]
SEEDS = list(range(54000, 56000))   # 2000 fresh seeds
N_SHARDS = 16

# all current deployed fixes on by default; isolate the end-game defense
CONFIGS = {
    "endgame-defense OFF": {"USE_ENDGAME_DEFENSE": False},
    "endgame-defense ON":  {"USE_ENDGAME_DEFENSE": True},
}
REF = "endgame-defense OFF"
...
if __name__ == "__main__":
    ...
    print(f"A/B: {list(CONFIGS)} x {len(SEEDS)} fresh games vs mix "
    ...
    for label in CONFIGS:
        ...
        pieces = "  ".join(f"{nm} {peropp[nm][0]/peropp[nm][1]:.3f}"
                           for nm in OPP_NAMES)
        print(f"  {p:.3f} [{lo:.3f},{hi:.3f}]  {label:28s} | {pieces}", flush=True)
    ...
    for label in CONFIGS:
        ...
        print(f"  {label}: mean diff {md:+.4f}  z={z:+.2f}  "
              f"({better} seeds better / {worse} worse)  -> {verdict}", flush=True)
