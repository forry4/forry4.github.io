"""Sweep on the FIXED-tta base: cheapness discount (W_COST) for 0-point cards,
plus a W_SPREAD retest point now that tta is corrected.

W_COST: card_value /= (1 + W_COST * total_effective_cost) for PTS==0 cards, so the
cheaper of two equal-benefit 0-point cards is preferred. W_SPREAD retest checks
whether the L1 spread discount earns its keep once the tta bug (which accidentally
favored spread) is gone. Paired vs the all-off reference on the SAME fresh seeds vs
the A/B/C/C2 mix. ASCII output only (cp1252).
"""
import os
...
OPP_NAMES = ["A", "B", "C", "C2"]
SEEDS = list(range(64000, 65000))   # 1000 fresh seeds (disjoint from prior runs)
N_SHARDS = 16

# USE_TTA_GREEDY stays at its module default (True) -> fixed-tta base for all configs.
CONFIGS = {
    "ref (all off)":          {},
    "W_COST 0.05":            {"W_COST": 0.05},
    "W_COST 0.1":             {"W_COST": 0.1},
    "W_COST 0.2":             {"W_COST": 0.2},
    "W_COST 0.4":             {"W_COST": 0.4},
    "W_SPREAD 0.4 (fixed)":   {"W_SPREAD": 0.4},
}
REF = "ref (all off)"
...
if __name__ == "__main__":
    ...
    print(f"sweep on fixed-tta base {list(CONFIGS)} x {len(SEEDS)} fresh games vs mix "
