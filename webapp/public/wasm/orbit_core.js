 ...
 * @param {number} remaining_turn_budget
 * @param {number} seed
 * @returns {string}
 */
export function orbit_choose_move_json(observation_json, legal_moves_json, memory_json, remaining_turn_budget, seed) {
    let deferred4_0;
    let deferred4_1;
    try {
        ...
        const ret = wasm.orbit_choose_move_json(ptr0, len0, ptr1, len1, ptr2, len2, remaining_turn_budget, seed);
        deferred4_0 = ret[0];
        deferred4_1 = ret[1];
 ...
 * @returns {string}
 */
export function orbit_serving_manifest_json() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.orbit_serving_manifest_json();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('orbit_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
