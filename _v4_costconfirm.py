"""Confirm the cost-model bundle at higher N: W_COST and W_SPREAD alone and COMBINED,
on the fixed-tta base, vs all-off. 2000 fresh paired seeds vs the A/B/C/C2 mix.
Decides whether to ship them (flip on) and whether they're additive. ASCII only.
"""
import os
...
OPP_NAMES = ["A", "B", "C", "C2"]
SEEDS = list(range(67000, 69000))   # 2000 fresh seeds (disjoint from prior runs)
N_SHARDS = 16
...
CONFIGS = {
    ...
    "both (cost.4+spr.4)":  {"W_COST": 0.4, "W_SPREAD": 0.4},
}
REF = "ref (off)"
...
if __name__ == "__main__":
    ...
    print(f"cost-model confirm {list(CONFIGS)} x {len(SEEDS)} fresh games vs mix "
