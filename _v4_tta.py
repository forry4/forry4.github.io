"""A/B: the turns_to_afford fix (old closed form vs greedy simulation).

The old form under-counted costs concentrated in one color that also need others
(2g+1r -> 1 but really 2; 4w+1u -> 2 but really 3). turns_to_afford feeds the
tempo discount, take-target selection, and reserve-threat detection, so this is a
broad change -- A/B it on its own. Paired on the SAME fresh seeds vs the A/B/C/C2
mix. Toggles valuation.USE_TTA_GREEDY. ASCII output only (cp1252).
"""
import os
...
OPP_NAMES = ["A", "B", "C", "C2"]
SEEDS = list(range(63000, 64000))   # 1000 fresh seeds (disjoint from prior runs)
N_SHARDS = 16

CONFIGS = {
    "tta old (buggy)":    {"USE_TTA_GREEDY": False},
    "tta greedy (fixed)": {"USE_TTA_GREEDY": True},
}
REF = "tta old (buggy)"
...
def _run(overrides, seeds):
    ...
    for k, v in overrides.items():
        setattr(V, k, v)                       # the tta flag lives on valuation
...
if __name__ == "__main__":
    ...
    print(f"A/B turns_to_afford {list(CONFIGS)} x {len(SEEDS)} fresh games vs mix "
