...
def samples(game, *, per_seat=4):
    ...
    for seat in (0,1):
        ...
        for k in range(per_seat):
            ...
            labels.append(0.5 if game["winner"] is None else float(game["winner"]==seat))
...
def fit_vocabulary(directory,manifest):
    ...
    for index,item in enumerate(manifest["games"]):
        ...
        if (index+1)%384==0:print(json.dumps({"vocabulary_games":index+1}),flush=True)
...
def train(directory,development,output,epochs,device,resume=None,indexed_features=False,fused_adam=False,training_seed=9400):
    ...
    train_manifest=load_manifest(directory)
    ...
    if {g["seed"] for g in train_manifest["games"]}&{g["seed"] for g in dev_manifest["games"]}:
        raise ValueError("Training/development seed overlap")
    torch.set_num_threads(2)
    torch.manual_seed(training_seed)
    fingerprints={"train":hashlib.sha256(json.dumps(train_manifest,sort_keys=True).encode()).hexdigest(),
    ...
    if resume:
        ...
        model=AttentionValue(fit_vocabulary(directory,train_manifest),ModelConfig(indexed_features=indexed_features)).to(device)
    ...
    def prepared(root,item):
        ...
        inputs,labels=samples(read_game(root,item))
    ...
    for root,manifest in [(directory,train_manifest),(development,dev_manifest)]:
