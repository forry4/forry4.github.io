...
Per-area detail lives in a `AGENTS.md` next to the code, loaded when you read files there:

| File | Covers |
|---|---|
| [`games/spender/AGENTS.md`](games/spender/AGENTS.md) | Spender engine/backend, the AI variant zoo, Puzzle mode, and `Spender.jsx` (also the site shell) |
| [`games/castles_of_crimson/AGENTS.md`](games/castles_of_crimson/AGENTS.md) | CoC engine contract, AI tiers, frontend layout maths |
| [`games/wherewolf/AGENTS.md`](games/wherewolf/AGENTS.md) | WW roles, redaction matrix, night conductor |
| [`games/spender_duel/AGENTS.md`](games/spender_duel/AGENTS.md) | Duel engine, hidden info, and the current coherent/minimax search |
| [`games/dontminion/AGENTS.md`](games/dontminion/AGENTS.md) | Dontminion (Dominion) frame-stack engine, the frozen effects API, multi-bot server, decision-prompt frontend |
| [`games/dissonance/AGENTS.md`](games/dissonance/AGENTS.md) | Dissonance — parity trick-taking rules, the Rust reference + parity gate, auction/scoring calibration, the five modes (classic / skat / minor / dummy / quartet), and the browser-served Hard tier |
| [`games/rag_tag/AGENTS.md`](games/rag_tag/AGENTS.md) | Rag Tag (Tag Team) — the simultaneous turn resolution, the generated fighter data and where it came from, the two-pass declaration |
| [`shared/AGENTS.md`](shared/AGENTS.md) | Shared frontend kits + URL routing |
| [`books/AGENTS.md`](books/AGENTS.md) | The Books feature |
| [`bggfilter/AGENTS.md`](bggfilter/AGENTS.md) | BGG Filter — the BoardGameGeek harvest + the frontend-only filter page |
...
games/
  ...
  dissonance/          # Dissonance — 2p parity trick-taking. engine.py is a PORT of
                       ...
                       #   left holding) — see its AGENTS.md
...
## Cross-game AI facts (do not relitigate — per-game tiers in each package's `AGENTS.md`)
...
- **Rules/engine unit tests are the most valuable to protect** — each game has them (per-package
  `AGENTS.md` lists what each covers), plus `core/tests/` (db/auth/ratelimit/retention, in-memory
