...
- **THE OFFLINE HUB (`/offline`) IS THE ONE DOOR WITH NO CONNECTION (2026-09-26)** — the only route
  the boot gate lets through without the backend, reached from the loading screen's "Use offline"
  and the menu's Offline tile. Two sections: **Play vs AI** (the local games) and **Read** —
  Spender Puzzles, BGG Filter, Books, Notes, your profile — each READ-ONLY from a copy this device
  keeps, and each returning to the hub on Back. Where the copies come from:
  - **Downloads are PACKS** (`OFFLINE_PACKS` in Spender.jsx, mechanics in
    `shared/offlinePacks.js`): static files through the service worker, the screen's own LAZY
    CHUNK (imported so the SW keeps it — the hub used to fetch a game's wasm but never its code),
    and a `warm` step. A download is RECORDED against the build and re-fetched after every deploy,
    because each deploy's SW deletes the previous cache — downloads used to vanish within a day.
  - **The puzzle bank is a static file** (`webapp/public/data/puzzles.json`, GENERATED by
    `python -m games.spender.puzzle.export_static`; `test_puzzle_static.py` fails when it is
    stale). A puzzle is a scripted line checked by comparison, so it needs no engine or backend.
  - **Books, Notes and Profile keep their own copies** whenever they load online (Books
    `books.offline.v1`, Profile `profile.history.<uid>`, Notes the tree in localStorage plus
    bodies and pictures in IndexedDB `forrest-notes`, `notes/offlineStore.js`), and show them
    read-only when the server cannot be reached. **Notes and the profile are PRIVATE, so a real
    sign-out wipes them** (an expired session does not). `screens.mjs` `offlineRead` takes each
    copy online, blocks the API origin, and reads them back.
